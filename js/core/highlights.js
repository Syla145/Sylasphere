(function () {
  ...
  function compute(state) {
    ...
    if (top) cards.push({ icon: '🎯', title: 'Treffsicher', names: top.names, detail: `bei ${top.value} von ${played} Fragen gepunktet` });
    const king = leaders(buzzer, names);
    if (king) cards.push({ icon: '⚡', title: 'Buzzer-König', names: king.names, detail: `${king.value} ${king.value === 1 ? 'Buzzer-Frage' : 'Buzzer-Fragen'} gewonnen` });
    if (closest) cards.push({ icon: '📏', title: 'Knappste Schätzung', names: [names[closest.pid]], detail: closest.rel === 0 ? `exakt ${numberFormat.format(closest.target)}${closest.unit ? ` ${closest.unit}` : ''} bei „${short(closest.text)}“` : `${numberFormat.format(closest.value)} statt ${numberFormat.format(closest.target)}${closest.unit ? ` ${closest.unit}` : ''} bei „${short(closest.text)}“` });
    if (rocket) cards.push({ icon: '🚀', title: 'Punkte-Rakete', names: [names[rocket.pid]], detail: `+${Math.round(rocket.points)} P bei „${short(rocket.text)}“` });
    const unlucky = leaders(zeros, names, 2);
    if (unlucky && played >= 3) cards.push({ icon: '🍀', title: 'Pechvogel des Abends', names: unlucky.names, detail: `${unlucky.value}× leer ausgegangen – nächstes Mal!` });
